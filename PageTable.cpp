...
int PageTable::swapper(VirtualMemory* mem) {
	...
	int* frame_out =
			this->_outerPageTable[swap_dir].get_table_address()[swap_pg].get_page_address();
...
int* PageTable::GetPage(unsigned int adr, VirtualMemory* mem) {
	...
	if (!(this->_outerPageTable[dir].is_valid())) //directory valid is 0
	{
		page_fault = 1;
	...
	else if (!((this->_outerPageTable[dir].get_table_address())[pg].is_valid())) //page valid is 0
	{
		page_fault = 1;
		if (mem->freeFramesList.empty()) //check if there are avilable frame
		{
	...
	{
		page_fault = 0;
		if (this->_outerPageTable[dir].get_table_address()[pg].is_swapped()) // check if page is pointed to swap device
		{
